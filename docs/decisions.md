...
Đã làm ở `88a4f89`. Luật quyết định từng có **bốn** bản chép tay; đổi ngưỡng
...
thấy `await` thì kết luận "không giữ luồng". Đã sửa cả ba ở `0b0888d`.
...
Lần thứ ba theo nếp của `ad77085` và `3ecdbe8`. Nhưng lần này phải nói thẳng một
...
`onnxEngine.js` có hai byte NUL thật trong mã nguồn — khoá cache ở `3270582` viết
...
tiên** (`db46142`): `bpe.js` dùng byte NUL thật làm dấu ngăn trong khoá bảng
...
Cache theo câu (`3270582`) lưu **cả câu** cho mọi phiên bản. Với bài dài không dấu
...
* Code mới với cache **tắt** cho y hệt `c3abae1` trên 120 văn bản, cùng số lượt
...
`node dev/measure-chrome.mjs`, commit `7c11ec1`, sáu lượt xen kẽ, máy Ryzen 9 8945HS
...
Ba biến thể, chín lượt xen kẽ: **tắt, A, O** × 3. **A** = bản đang ship, gói ở `fd0fd53`
(`dist/soat-A-fd0fd53.zip`). **O** = bản offscreen. Mỗi lượt, A và O chạy y hệt nhau:
...
`1f838be`. Lần chạy đầu trong Chrome headless: offscreen được tạo, Worker chạy, message
...
`node dev/measure-chrome.mjs --plan 38`, công cụ ở `a6f287f`, bản O ở `1f838be`, A ở
`fd0fd53`. Chín lượt: tắt / A / O × 3. A/A bộ nhớ ba lượt tắt: **2,3 MB/tab** (cần ≤ 10).
...
mỗi bản, thêm ở `7a2a34f`, đổi ở `665f46c`, gỡ khỏi HEAD ở `375764b`). GitHub từ chối file
...
Không dùng `--prune-empty`: commit `375764b` ("Blob 311MB vẫn nằm trong HEAD — commit trước
...
`375764b`) — bản fp32 vẫn dùng để xuất lại ONNX được.
...
ghi ở `5302291`, trước khi viết một dòng code". Rewrite đổi hash của mọi commit từ `7a2a34f`
